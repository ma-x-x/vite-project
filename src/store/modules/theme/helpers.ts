...
function getThemeSettings() {
	return localStorage.get(EnumStorageKey["theme-settings"]);
}

/** 获取缓存中的主题配置 */
export function setThemeSettings(settings: Theme.Setting) {
	return localStorage.set(EnumStorageKey["theme-settings"], settings);
}

/** 清除缓存配置 */
export function clearThemeSettings() {
	localStorage.remove(EnumStorageKey["theme-settings"]);
}
